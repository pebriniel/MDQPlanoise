...
function slider_frontend($atts){
	...
	while ( $loop->have_posts() ) {
		...
		if ( '' != get_the_post_thumbnail(get_the_ID(), $atts['image_size']) ) {
			...
			$video_url = get_post_meta(get_the_ID(), 'slider_video_url', true);
			$video_aspect = get_post_meta(get_the_ID(), 'slider_video_aspect', true);
	...
	if(count($images) > 0){
		ob_start();
		?>
		<div id="slider_<?php echo $id; ?>" class="carousel slide" <?php if($atts['use_javascript_animation'] == '0'){ echo ' data-ride="carousel"'; } ?> data-interval="<?php echo $atts['interval']; ?>">

			<?php // First content - the carousel indicators
			if( count( $images ) > 1 ){ ?>
				<ol class="carousel-indicators">
				<?php foreach ($images as $key => $image) { ?>
					<li data-target="#slider_<?php echo $id; ?>" data-slide-to="<?php echo $key; ?>" <?php echo $key == 0 ? 'class="active"' : ''; ?>></li>
				<?php } ?>
				</ol>
			<?php } ?>
			...
			if( count( $images ) > 1 ){
				if($atts['showcontrols'] === 'true' && ($atts['twbs'] == '3' || $atts['twbs'] == '4')) { ?>
					<a class="left carousel-control" href="#slider_<?php echo $id; ?>" data-slide="prev"><span class="glyphicon glyphicon-chevron-left"></span></a>
					<a class="right carousel-control" href="#slider_<?php echo $id; ?>" data-slide="next"><span class="glyphicon glyphicon-chevron-right"></span></a>
				<?php } else if($atts['showcontrols'] === 'true'){ ?>
					<a class="left carousel-control" href="#slider_<?php echo $id; ?>" data-slide="prev">‹</a>
					<a class="right carousel-control" href="#slider_<?php echo $id; ?>" data-slide="next">›</a>
				<?php } else if($atts['showcontrols'] === 'custom' && ($atts['twbs'] == '3' || $atts['twbs'] == '4') &&  $atts['customprev'] != '' &&  $atts['customnext'] != ''){ ?>
					<a class="left carousel-control" href="#slider_<?php echo $id; ?>" data-slide="prev"><span class="<?php echo $atts['customprev'] ?> icon-prev"></span></a>
					<a class="right carousel-control" href="#slider_<?php echo $id; ?>" data-slide="next"><span class="<?php echo $atts['customnext'] ?> icon-next"></span></a>
				<?php }
			} ?>
		...
		<script type="text/javascript">
			jQuery(document).ready(function() {
				jQuery('#slider_<?php echo $id; ?>').carousel({
        ...
        <?php }
				...
				if (count($video_providers) > 0) { ?>
					<script>
						var $myCarousel = jQuery('#slider_<?php echo $id; ?>');
		...
		$output = '<!-- CPT Bootstrap Carousel - no images found for #slider_'.$id.' -->';
	}
