...
register_activation_hook(__FILE__, 'slider_set_options');
function slider_set_options (){
	$defaults = array(
		'interval' => '5000',
		'showtitle' => 'true',
		'showcaption' => 'true',
	...
	add_option('slider_settings', $defaults);
}
// Clean up on uninstall
register_activation_hook(__FILE__, 'slider_deactivate');
function slider_deactivate(){
...
class slider_settings_page {
	...
	public function add_plugin_page() {
		add_submenu_page('edit.php?post_type=slider', __('Settings', 'cpt-bootstrap-carousel'), __('Settings', 'cpt-bootstrap-carousel'), 'manage_options', 'cpt-bootstrap-carousel', array($this,'create_admin_page'));
	}
	...
	public function create_admin_page() {
		...
		<p><?php printf(__('You can set the default behaviour of your carousels here. Most of these settings can be overridden by using %s shortcode attributes %s.', 'cpt-bootstrap-carousel'),'<a href="http://wordpress.org/plugins/cpt-bootstrap-carousel/" target="_blank">', '</a>'); ?></p>
				...
				<?php
						settings_fields( 'slider_settings' );
	...
	public function page_init() {
		...
		add_settings_field(
				...
				'cpt-bootstrap-carousel', // Page
				'slider_settings_link_buttons' // Section
		);
		add_settings_field(
				'link_button_text', // ID
		...
		add_settings_field(
				...
				'cpt-bootstrap-carousel', // Page
				'slider_settings_markup' // Section
		);
		add_settings_field(
				'customnext', // ID
	...
	public function showtitle_callback() {
		...
		print '<select id="showtitle" name="slider_settings[showtitle]">
			<option value="true"'.$slider_showtitle_t.'>'.__('Show', 'cpt-bootstrap-carousel').'</option>
			<option value="false"'.$slider_showtitle_f.'>'.__('Hide', 'cpt-bootstrap-carousel').'</option>
		</select>';
	}
	public function showcaption_callback() {
		if(isset( $this->options['showcaption'] ) && $this->options['showcaption'] == 'false'){
			$slider_showcaption_t = '';
			$slider_showcaption_f = ' selected="selected"';
		} else {
			$slider_showcaption_t = ' selected="selected"';
			$slider_showcaption_f = '';
		}
		print '<select id="showcaption" name="slider_settings[showcaption]">
			<option value="true"'.$slider_showcaption_t.'>'.__('Show', 'cpt-bootstrap-carousel').'</option>
			<option value="false"'.$slider_showcaption_f.'>'.__('Hide', 'cpt-bootstrap-carousel').'</option>
		</select>';
	}
	public function showcontrols_callback() {
		if(isset( $this->options['showcontrols'] ) && $this->options['showcontrols'] == 'false'){
			$slider_showcontrols_t = '';
			$slider_showcontrols_f = ' selected="selected"';
			$slider_showcontrols_c = '';
		} else if(isset( $this->options['showcontrols'] ) && $this->options['showcontrols'] == 'true'){
			$slider_showcontrols_t = ' selected="selected"';
			$slider_showcontrols_f = '';
			$slider_showcontrols_c = '';
		} else if(isset( $this->options['showcontrols'] ) && $this->options['showcontrols'] == 'custom'){
			$slider_showcontrols_t = '';
			$slider_showcontrols_f = '';
			$slider_showcontrols_c = ' selected="selected"';
		}
		print '<select id="showcontrols" name="slider_settings[showcontrols]">
			<option value="true"'.$slider_showcontrols_t.'>'.__('Show', 'cpt-bootstrap-carousel').'</option>
			<option value="false"'.$slider_showcontrols_f.'>'.__('Hide', 'cpt-bootstrap-carousel').'</option>
			<option value="custom"'.$slider_showcontrols_c.'>'.__('Custom', 'cpt-bootstrap-carousel').'</option>
		</select>';
	}
	public function orderby_callback() {
		...
		print '<select id="orderby" name="slider_settings[orderby]">';
	...
	public function order_callback() {
		if(isset( $this->options['order'] ) && $this->options['order'] == 'DESC'){
			$slider_showcontrols_a = '';
			$slider_showcontrols_d = ' selected="selected"';
		} else {
			$slider_showcontrols_a = ' selected="selected"';
			$slider_showcontrols_d = '';
		}
		print '<select id="order" name="slider_settings[order]">
			<option value="ASC"'.$slider_showcontrols_a.'>'.__('Ascending', 'cpt-bootstrap-carousel').'</option>
			<option value="DESC"'.$slider_showcontrols_d.'>'.__('Decending', 'cpt-bootstrap-carousel').'</option>
		</select>';
	}
	public function category_callback() {
		$cats = get_terms('carousel_category');
		print '<select id="orderby" name="slider_settings[category]">
	...
	public function twbs_callback() {
		...
		print '<select id="twbs" name="slider_settings[twbs]">
			<option value="2"'.$slider_twbs2.'>2.x</option>
			<option value="3"'.$slider_twbs3.'>3.x (Default)</option>
	...
	public function image_size_callback() {
		...
		print '<select id="image_size" name="slider_settings[image_size]">
			<option value="full"';
	...
	public function use_background_images_callback() {
		print '<select id="use_background_images" name="slider_settings[use_background_images]">';
	...
	public function background_images_height_callback() {
		printf('<input type="text" id="background_images_height" name="slider_settings[background_images_height]" value="%s" size="15" />',
	...
	public function use_javascript_animation_callback() {
		print '<select id="use_javascript_animation" name="slider_settings[use_javascript_animation]">';
		print '<option value="1"';
	...
	public function background_images_style_size_callback() {
		print '<select id="select_background_images_style_size" name="slider_settings[select_background_images_style_size]">';
	...
	public function link_button_callback(){
		print '<select id="link_button" name="slider_settings[link_button]">';
		print '<option value="1"';
	...
	public function link_button_text_callback() {
			printf('<input type="text" id="link_button_text" name="slider_settings[link_button_text]" value="%s" size="20" />',
	...
	public function link_button_class_callback() {
			printf('<input type="text" id="link_button_class" name="slider_settings[link_button_class]" value="%s" size="20" />',
...
if( is_admin() ){
		$slider_settings_page = new slider_settings_page();
}

// Add settings link on plugin page
function slider_settings_link ($links) {
	$settings_link = '<a href="edit.php?post_type=slider&page=cpt-bootstrap-carousel">'.__('Settings', 'cpt-bootstrap-carousel').'</a>';
	array_unshift($links, $settings_link);
	return $links;
}
$slider_plugin = plugin_basename(__FILE__);
add_filter("plugin_action_links_$slider_plugin", 'slider_settings_link' );
